  ...
  const handleSubmit = (event) => {
    ...
    return (
        ...
        <div className='sign-template-text'>
            New to PrimeXBT? <Link to="/sign-up" className='link'>Register</Link> 
        </div>
        ...
        {showError && <p style={{color: 'red', fontSize:14, marginBottom:30}}>Wrong email or password!</p>}
        <Link to="/" className='sign-template-text link' style={{display: 'inline-block' , marginBottom:30}}>Forgot password?</Link> 
        <div style={{display:'flex', flexDirection: 'column'}}> 
        <CustomButtonPrimary type='submit' disabled={false}> Log in </CustomButtonPrimary>
        </div>
        </form>
    );
}

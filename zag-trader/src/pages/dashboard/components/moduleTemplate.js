  ...
  return (
    <CustomCard>
      <div className="customCard-header">
        <div className="customCard-header-title" onClick={handleClick}>
          {" "}
          {props.icon} &nbsp; &nbsp; <span> {props.title}</span> &nbsp;{" "}
          {show ? <FaAngleDown size={24} /> : <FaAngleUp size={24} />}{" "}
        </div>
        {props.summery ? (
          <div className="customCard-header-summery">
            {" "}
            <div>Total equity</div>{" "}
            <div style={{ color: "#fff" }}>≈ {props.summery} USD</div>
          </div>
        ) : (
          <div></div>
        )}
      ...
      {show && props.onboardingTitle && (
        <div className="onboardingBanner">
          <div
            className="onboardingBanner-pic"
            style={{ background: `url(${props.pic}) no-repeat` }}
          ></div>
          ...
          <div className="onboardingBanner-close">
            <FaTimes color="#ccc" size={18} />
          </div>
        </div>
      )}
    </CustomCard>
